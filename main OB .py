...
##################################################IT USED FOR Voice Output PROCCESSING ###################################################################
##################################################IT USED FOR IMAGE PROCCESSING ###################################################################
def null():
    global label_list
    label_list = ""
    if(condition):
        threading.Timer(1, null).start()
    ...
    while (True):
        ...
        try:
            for classId, conf, box in zip(classIds.flatten(), confs.flatten(), bbox):#bounding boxes
                ...
                label_list += f'{classNames[classId-1].upper()} '
        except:
            print('...')
...
def voice():
    ...
    # engine.setProperty('voices', 180)
    engine.setProperty('voice', voices[1].id)
    labelset = set(labels.split(' '))
    labelstr = str(labelset)
    print(labelstr," set")
    engine.say(labelstr) 
    engine.runAndWait()
    engine.stop()
    ...
    if(condition):
        threading.Timer(0.001, voice).start()
        ...
        if len(confs) > 0:       
            for classIds,confs,boxes in zip(classIds.flatten(), confs.flatten(), bbox):
                ...
                cv2.putText(imges,f'{classNames[classIds-1].upper()} {round(confs*100,2)}',(boxes[0]+box,boxes[1]+box),
                    font,f_size,
                    Color,f_bold)
                label_list +=(" " +(classNames[classIds-1])+" Detected")
...
def show_vdo():
    ...
    Thread(target = getlabel,args=(cap,)).start()
